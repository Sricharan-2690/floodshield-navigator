...
function RiskMapClickHandler({
  ...
  useMapEvents({
    click(e) {
      ...
      const floodScore = computeFloodScore(rawValue, rainFactor, "realtime");
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const rainRes = await fetch(
        "https://api.open-meteo.com/v1/forecast?latitude=17.406&longitude=78.477&hourly=rain&forecast_days=1"
      );
  ...
  useEffect(() => {
    ...
    const layer = new GeoRasterLayer({
      ...
      pixelValuesToColorFn: (values: number[]) => {
        ...
        return floodColorRamp(computeFloodScore(p, rainFactor, "realtime"));
      },
    });
    layer.addTo(map);
  ...
  return (
    <div className="fixed top-4 right-4 z-[1004]" style={{ marginTop: "3.5rem" }}>
      <div className="fs-glass-strong rounded-xl p-1 flex gap-1 shadow-elev">
        ...
        <button onClick={() => navigate("/map/susceptibility")}
    ...
    <div className="absolute left-4 top-40 z-[1000] fs-glass-strong rounded-2xl p-4 min-w-[180px]">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <div className="flex size-8 items-center justify-center rounded-lg bg-primary/10"><CloudRain className="size-4 text-primary" /></div>
          <div><div className="text-xs text-muted-foreground">Normalized</div><div className="font-mono text-sm font-semibold text-foreground">{(rainData.rainFactor * 100).toFixed(0)}%</div></div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex size-8 items-center justify-center rounded-lg bg-primary/10"><Droplets className="size-4 text-primary" /></div>
          <div><div className="text-xs text-muted-foreground">24h Rain</div><div className="font-mono text-sm font-semibold text-foreground">{rainData.rain24h.toFixed(1)} mm</div></div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex size-8 items-center justify-center rounded-lg bg-primary/10"><Zap className="size-4 text-primary" /></div>
          <div><div className="text-xs text-muted-foreground">Peak Hour</div><div className="font-mono text-sm font-semibold text-foreground">{rainData.rainMax.toFixed(1)} mm</div></div>
        </div>
      </div>
    </div>
  );
...
export default function MapRisk() {
  const [rainData, setRainData] = useState<RainData>({ rainFactor: 0, rain24h: 0, rainMax: 0 });
